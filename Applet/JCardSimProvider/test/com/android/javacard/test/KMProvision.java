...
public class KMProvision {
  ...
  public static void provisionCmd(CardSimulator simulator,
    ...
    Assert.assertEquals(KMError.OK, KMTestUtils.decodeError(decoder,
        provisionSigningCertificate(simulator, encoder, decoder)));
    Assert.assertEquals(KMError.OK, KMTestUtils.decodeError(decoder,
  ...
  private static final byte INS_PROVISION_ATTESTATION_CERT_DATA_CMD = 0 + 2; //0x02
  public static ResponseAPDU provisionSigningCertificate(CardSimulator simulator, KMEncoder encoder,
      KMDecoder decoder) {
    short arrPtr = KMArray.instance((short) 3);
    short innerArrPtr = KMArray.instance((short) 2);

    short byteBlobPtr1 = KMByteBlob.instance(
        (short) (kEcAttestCert.length));
    Util.arrayCopyNonAtomic(kEcAttestCert, (short) 0,
        KMByteBlob.cast(byteBlobPtr1).getBuffer(),
        KMByteBlob.cast(byteBlobPtr1).getStartOff(),
        (short) kEcAttestCert.length);

    short byteBlobPtr2 = KMByteBlob.instance(
        (short) (kEcAttestRootCert.length));
    Util.arrayCopyNonAtomic(kEcAttestRootCert, (short) 0,
        KMByteBlob.cast(byteBlobPtr2).getBuffer(),
        KMByteBlob.cast(byteBlobPtr2).getStartOff(),
        (short) kEcAttestRootCert.length);
    ...
    //     (short) kEcAttestRootCert.length);
    //KMArray.cast(arrPtr).add((short) 0, encodedInnerArrayPtr);
    ...
    CommandAPDU apdu = KMTestUtils.encodeApdu(encoder,
        (byte) INS_PROVISION_ATTESTATION_CERT_DATA_CMD, encodedData);
    // print(commandAPDU.getBytes());
    ResponseAPDU response = simulator.transmitCommand(apdu);
