...
public class KMPoolManager {
 ...
 // Extra 4 algorithms are used to support TRUSTED_CONFIRMATION_REQUIRED feature.
  ...
  private KMKeyObject createKeyObjectInstance(byte alg) {
    ...
    switch (alg) {
      case AES_128:
        keyObject = (AESKey) KeyBuilder.buildKey(
      ...
      case KMType.EC:
    	...
    	break;
      case KMType.HMAC:
    	keyObject = (HMACKey) KeyBuilder.buildKey(KeyBuilder.TYPE_HMAC_TRANSIENT_RESET, (short) 512,
    	            false);
        break;
      default:
    }
  ...
  private boolean isKeyObjectBusy(Object obj) {
    short index = 0;
    while (index < MAX_OPERATION_INSTANCES) {
      if (((KMOperationImpl) operationPool[index]).isKeyObjectMatches(obj)
    		  || ((KMOperationImpl) hmacSignOperationPool[index]).isKeyObjectMatches(obj)) {
        return true;
      }
      index++;
    }
