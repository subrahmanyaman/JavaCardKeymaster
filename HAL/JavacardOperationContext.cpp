...
ErrorCode OperationContext::update(uint64_t operHandle, const std::vector<uint8_t>& actualInput,
    ...
    if (input.size() > MAX_ALLOWED_INPUT_SIZE) {
        ...
        for (int i = 0; i < noOfChunks; i++) {
            ...
            if (ErrorCode::OK !=
                (errorCode = handleInternalUpdate(operHandle, newInput, Operation::Update, cb))) {
                return errorCode;
            }
        }
        if (extraData > 0) {
            ...
            if (ErrorCode::OK !=
                (errorCode = handleInternalUpdate(operHandle, finalInput, Operation::Update, cb))) {
                return errorCode;
            }
        }
    } else {
        if (ErrorCode::OK !=
            (errorCode = handleInternalUpdate(operHandle, input, Operation::Update, cb))) {
            return errorCode;
        }
    }
    return errorCode;
...
ErrorCode OperationContext::finish(uint64_t operHandle, const std::vector<uint8_t>& actualInput,
    ...
    if (input.size() > MAX_ALLOWED_INPUT_SIZE) {
        ...
        for (int i = 0; i < noOfChunks; i++) {
            ...
            if (extraData == 0 && (i == noOfChunks - 1)) {
                ...
                if (ErrorCode::OK != (errorCode = handleInternalUpdate(
                                          operHandle, newInput, Operation::Finish, cb, true))) {
                    return errorCode;
                }
        ...
        if (ErrorCode::OK !=
            (errorCode = handleInternalUpdate(operHandle, input, Operation::Finish, cb, true))) {
            return errorCode;
        }
    }
    return errorCode;
 ...
 * data. It first Copies the data to the out buffer from buffered data and then the remaining from
 * the input data. If the buffered data is empty then it copies data to out buffer from only input
 * and similarly if the input is empty then it copies from only buffer. Incase if only a portion of
 * the input data is consumed then the remaining portion of input data is buffered. For AES/TDES
 * Encryption/Decryption operations with PKCS7 padding and for AES GCM Decryption operations a block
 * size of data and tag length of data is always buffered respectively. This is done to make sure
 * that there will be always a block size of data left for finish operation so that the Applet may
 * remove the PKCS7 padding if any or get the tag data for AES GCM operation for authentication
 * purpose. Once the data from the buffer is consumed then the buffer is cleared. No Buffering is
 * done for other modes of operation.
 */
ErrorCode OperationContext::bufferData(uint64_t operHandle, std::vector<uint8_t>& input,
    ...
    if (opr == Operation::Finish) {
        ...
        if (operationTable[operHandle].info.pad == PaddingMode::PKCS7) {
            ...
            if ((data.buf_len + input.size()) > operationTable[operHandle].info.macLength) {
                dataToSELen =
                    (data.buf_len + input.size()) - operationTable[operHandle].info.macLength;
            }
        } else {
            /* No Buffering */
    ...
    if (dataToSELen > 0) {
        ...
        if (bufferLengthConsumed < data.buf_len) {
            ...
            memmove(data.buf, (data.buf + bufferLengthConsumed),
...
ErrorCode OperationContext::handleInternalUpdate(uint64_t operHandle, std::vector<uint8_t>& data,
    ...
    if (Algorithm::AES == operationTable[operHandle].info.alg ||
        ...
        /*Symmetric */
        if (ErrorCode::OK != (errorCode = bufferData(operHandle, data, opr, out))) {
            return errorCode;
        }
        ...
        if (operationTable[operHandle].info.purpose == KeyPurpose::DECRYPT ||
            ...
            if (finish) {
                ...
                out.insert(out.end(), operationTable[operHandle].data.buf,
                           operationTable[operHandle].data.buf +
